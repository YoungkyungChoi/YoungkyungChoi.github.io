...
/*
  ...
  separate object ("blob").
*/
p5.prototype.VidaBlob = function() {
  /*
  ...
  */
  this.creationTime = millis(); this.creationFrameCount = frameCount;
  /*
...
/*
  Main VIDA pseudoclass.
*/
p5.prototype.Vida = function() {
  /* 
  ...
  */
  this.currentImage    = createGraphics(10, 10);
  this.backgroundImage = createImage(10, 10);
  this.differenceImage = createImage(10, 10);
  this.thresholdImage  = createImage(10, 10);
  /*
...
p5.prototype.Vida.prototype.drawBlobs = function(
  ...
  */
  ...
  translate(_x, _y); // translate coords
  ...
  pop(); // restore drawing style and font
}
  ...
  */
  ...
  for(var i = 0; i < this.activeZones.length; i++) {
    ...
    if(this.activeZones[i].isEnabledFlag) {
      stroke(255, 0, 0);
      if(this.activeZones[i].isMovementDetectedFlag) fill(255, 0, 0, 128);
      else noFill();
    }
    else {
      stroke(0, 0, 255);
      ...
      */
      if(this.activeZones[i].isMovementDetectedFlag) fill(0, 0, 255, 128);
      else noFill();
    }
    rect(temp_coord_x, temp_coord_y, temp_coord_w, temp_coord_h);
    // print id
    noStroke();
    if(this.activeZones[i].isEnabledFlag) fill(255, 0, 0);
    else fill(0, 0, 255);
    text(this.activeZones[i].id, temp_coord_x, temp_coord_y - 1);
  }
  ...
  for(var i = 0; i < this.activeZones.length; i++) {
    ...
    */
    ...
    if(
      ...
      this.activeZones[i].changedTime = millis();
      this.activeZones[i].changedFrameCount = frameCount;
...
p5.prototype.Vida.prototype.update = function(_image) {
  if(this.updateImageProcessor(_image)) {
    ...
    this.lastUpdateTime = millis(); this.lastUpdateFrameCount = frameCount;
  }
  else {
    console.log(
...
p5.prototype.Vida.prototype.updateImageProcessor = function(_image) {
  ...
  */
  ...
  this.thresholdImage.filter(THRESHOLD, this.imageFilterThreshold);
